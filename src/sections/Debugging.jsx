...
const CODE_LINES = [
  ...
  {
    id: 3,
    tokens: [{ t: "string", v: "    `/api/users/${userId}`" }],
    status: "normal",
  },
  ...
  },
  {
    ...
    status: "error",
    errorMsg: "Spent 3 hours here 😭",
  },
  { id: 7, tokens: [{ t: "plain", v: "}" }], status: "normal" },
  { id: 8, tokens: [], status: "empty" },
  {
    id: 9,
    tokens: [{ t: "comment", v: "// Call the function" }],
    status: "normal",
  },
  {
    id: 10,
...
function ErrorTooltip({ visible }) {
  return (
    ...
    >
      <div
        className="bg-[#1a0a0a] border border-red-500/40 rounded-xl px-3 py-2
        shadow-[0_0_20px_rgba(220,38,38,0.25)]"
      >
        <p className="font-mono text-[11px] text-red-300 leading-none">
          Spent 3 hours here 😭
        </p>
        <div
          className="absolute right-full top-1/2 -translate-y-1/2 mr-0
          border-8 border-transparent border-r-red-500/40"
          style={{ borderRightColor: "rgba(153,27,27,0.4)" }}
        />
      </div>
    </div>
  );
...
function CodeLine({ line, visible, isError, isFixed, glitching }) {
  ...
  return (
    ...
    >
      {/* Line number */}
      <span
        className={`font-mono text-[11px] w-10 text-right pr-4 shrink-0 select-none
      ...
      {/* Tokens */}
      <span
        className={`font-mono leading-7 flex flex-wrap items-center text-[12.5px] sm:text-[13px] flex-1 min-w-0`}
...
function ConfusionMsg({ emoji, text, visible, idx }) {
  return (
    ...
    >
      ...
      <p className="font-mono text-[12px] sm:text-[13px] text-red-200/75 leading-snug">
        {text}
      </p>
    </div>
  );
...
function ConsolePanel({ visible, errorVisible, fixedVisible }) {
  return (
    <div
      className={`rounded-xl overflow-hidden border border-white/5 transition-all duration-500
    ...
    >
      <div className="flex items-center gap-2 bg-[#0f1318] px-3 py-2 border-b border-white/5">
        ...
        <span className="font-mono text-[10px] text-white/20 ml-2 tracking-wider uppercase">
          Console
        </span>
      </div>
...
export default function DebuggingSection({ onComplete }) {
  ...
  const runSequence = () => {
    ...
    [-4, 4, -3, 3, -2, 2, -1, 1, 0].forEach((xVal, i) => {
      tl.to(
        editorRef.current,
        { x: xVal, duration: 0.055, ease: "none" },
        shakeStart + i * 0.055,
      );
    });
    ...
    tl.to(
      bgFlashRef.current,
      { opacity: 0.18, duration: 0.12, ease: "power2.in" },
      afterType + 0.75,
    );
    tl.to(
      bgFlashRef.current,
    ...
    tl.to(
      titleRef.current,
      { color: "#ef4444", duration: 0.15 },
      afterType + 0.75,
    );
    tl.to(
      titleRef.current,
      { color: "#ffffff", duration: 0.6 },
      afterType + 0.95,
    );
    ...
    for (let m = 1; m <= CONFUSION_MSGS.length; m++) {
      tl.call(
        () => setVisibleMsgs(m),
        [],
        afterType + 1.8 + CONFUSION_MSGS[m - 1].delay,
      );
    }
    ...
    [-3, 3, -2, 2, 0].forEach((yVal, i) => {
      tl.to(
        editorRef.current,
        { y: yVal, duration: 0.07, ease: "none" },
        bounceStart + i * 0.07,
      );
    });
  };
  ...
  return (
    <>
      <link
        href="https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=JetBrains+Mono:wght@300;400;600;700&display=swap"
        rel="stylesheet"
      />
      <style>{`
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(rgba(220,38,38,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(220,38,38,0.025) 1px,transparent 1px)",
        ...
        {glitching && (
          <div className="absolute inset-0 pointer-events-none overflow-hidden z-20">
            <div
              className="w-full h-0.5 bg-red-500/20"
        ...
        <div
          ref={bgFlashRef}
          className="absolute inset-0 bg-red-600 pointer-events-none opacity-0 z-10"
        />

        {/* ── HEADER ── */}
        <div
          ref={headerRef}
          className="opacity-0 text-center mb-8 sm:mb-10 relative z-10 w-full max-w-3xl px-2"
        >
          ...
          >
            ...
            <span
              ...
              style={{
                backgroundImage:
                  "linear-gradient(90deg,#ef4444,#dc2626,#f97316,#ef4444)",
          ...
          <p
            className="font-mono text-white/28 leading-relaxed max-w-md mx-auto"
            style={{ fontSize: "clamp(11px,1.3vw,13px)" }}
          >
            One typo. That's all it takes.
            <br />
            <span className="text-white/45">
              The bug was there all along, waiting.
            </span>
          </p>
          ...
          <div className="flex items-center justify-center gap-3 mt-5 flex-wrap">
            {[
              ...
              {
                label: "Debug",
                done: ["confusion", "fix", "success"].includes(phase),
              },
              {
                label: "Fixed!",
                done: ["success"].includes(phase),
                success: true,
              },
            ].map((b, i) => (
              <div
                key={i}
                className={`flex items-center gap-1.5 px-3 py-1 rounded-full border font-mono text-[10px]
              ...
              >
                {b.done && (
                  <span className="w-1.5 h-1.5 rounded-full bg-current animate-[pulse_1.5s_infinite]" />
                )}
                {b.label}
              </div>
            ))}
        ...
        <div
          ref={mainLayoutRef}
          className="relative z-10 w-full max-w-275 flex flex-col lg:flex-row gap-4 sm:gap-5"
        >
          {/* ── LEFT: VS CODE EDITOR ── */}
          <div ref={editorRef} className="opacity-0 flex-1 min-w-0">
            ...
            >
              ...
              <div className="flex items-center justify-between bg-[#0e1216] px-4 py-2.5 border-b border-white/5 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`font-mono text-[9px] tracking-widest uppercase transition-colors duration-500 ${phaseLabel.color}`}
                  >
                    {phaseLabel.text}
                  </span>
              ...
              <div className="bg-[#0b0f13] p-3 sm:p-4">
                <div className="space-y-0.5">
                  {CODE_LINES.map((line, idx) => {
                    ...
                    const isErr =
                      line.status === "error" && errorActive && !errorFixed;
              ...
              >
                ...
                >
                  <span
                    ...
                    style={{
                      animation:
                        phase !== "idle" ? "pulse 1.5s infinite" : "none",
                    }}
                  />
                  {errorActive && !errorFixed
                    ? "1 error, 0 warnings"
          ...
          <div
            ref={sideRef}
            className="opacity-0 flex flex-col gap-4 lg:w-72 xl:w-80"
          >
            {/* Confusion panel */}
            <div
              ...
              style={{
                boxShadow: confusionVisible
                  ? "0 0 40px rgba(127,0,0,0.18)"
                  : "none",
              }}
            >
              ...
              <div className="p-4 space-y-3 min-h-30">
                ...
                {confusionVisible &&
                  !successVisible &&
                  CONFUSION_MSGS.map((m, i) => (
                    <ConfusionMsg
                      key={i}
                      {...m}
                      visible={i < visibleMsgs}
                      idx={i}
                    />
                  ))}
                {successVisible && (
                  <div className="space-y-2.5 animate-[fadeIn_0.5s_ease]">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-mono text-[13px] text-emerald-400 font-semibold">
                        Finally it works!
                      </p>
                    </div>
            ...
            {errorActive && !errorFixed && !successVisible && (
              ...
              >
                ...
                <p className="font-mono text-[11px] text-white/40 leading-snug mb-3">
                  <span className="text-red-400">profiel</span> →{" "}
                  <span className="text-emerald-400">profile</span>
                </p>
                <div
                  className="flex items-center justify-center gap-2 bg-red-950/60 hover:bg-red-900/40
            ...
            <div className="rounded-2xl border border-white/5 bg-[#0a0d10] p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  {
                    label: "Errors",
                    value: errorFixed ? "0" : errorActive ? "1" : "0",
                    color:
                      errorActive && !errorFixed
                        ? "text-red-400"
                        : "text-white/35",
                  ...
                  {
                    label: "Fixes",
                    value: errorFixed ? "1" : "0",
                    color: errorFixed ? "text-emerald-400" : "text-white/35",
                  },
                  {
                    label: "Lines",
                    value: `${visibleLines}/${CODE_LINES.length}`,
                    color: "text-white/40",
                  },
                  {
                    label: "Coffees ☕",
                    value: confusionVisible ? "∞" : "0",
                    color: confusionVisible
                      ? "text-orange-400"
                      : "text-white/35",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
                    className="bg-[#0d1117] rounded-xl px-3 py-2.5 border border-white/5"
                  >
                    <p className="font-mono text-[9px] text-white/20 uppercase tracking-wider mb-1">
                      {s.label}
                    </p>
                    <p
                      className={`font-mono text-[16px] font-semibold transition-all duration-500 ${s.color}`}
                    >
                      {s.value}
                    </p>
                  </div>
        ...
        {successVisible && (
          <div className="relative z-10 w-full max-w-275 mt-5 animate-[fadeIn_0.7s_ease]">
            <div
              className="rounded-2xl border border-emerald-700/30 bg-emerald-950/25 px-5 py-4
